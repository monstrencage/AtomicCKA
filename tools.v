...
Instance max_proper : Proper (le ==> le ==> le) Nat.max.
Proof. intros x y L z t L';lia. Qed.
...
Class Semantics (A : Type) (B : Type) := interpret : A -> B.
...
Hint Rewrite @SizeLength @SizeCons @SizeNil @SizeApp @SizeMap: simpl_typeclasses.
...
Lemma compose_assoc {A B C D} (f : B -> A) (g : C -> B) (h : D -> C) :
  f ∘ (g ∘ h) ≃ (f ∘ g) ∘ h.
Proof. intro;unfold Basics.compose;reflexivity. Qed.
...
Global Instance compose_proper {A B C} :
  Proper (sequiv ==> sequiv ==> sequiv) (@Basics.compose A B C).
Proof. intros f1 f2 E__f g1 g2 E__g x;unfold Basics.compose;rewrite E__f,E__g;reflexivity. Qed.
...
Proof.
  ...
  - rewrite <-E;eauto.
Qed.
Global Instance bijective_ext_eq {A B} : Proper (@sequiv (A->B) _ ==> iff) bijective.
Proof. intros f g E;split;intros (h1&h2);split;rewrite E||rewrite <- E;assumption. Qed.
...
Global Instance map_injective_injective {A B: Set} (f : A -> B) :
  injective f -> injective (map f).
Proof.
  intros I;split;intro l;induction l as [|a l];intros [|b m];simpl;
...
Global Instance injective_compose {A B C} (f : B -> C) (g : A -> B) :
  injective f -> injective g -> injective (f ∘ g).
Proof.
  unfold Basics.compose; intros I1 I2;split.
  intros a b E;apply I1,I2 in E;assumption.
Qed.

Global Instance surjective_compose {A B C} (f : B -> C) (g : A -> B) :
  surjective f -> surjective g -> surjective (f ∘ g).
Proof.
  unfold Basics.compose; intros S1 S2;split.
  intros b. 
  destruct (is_surjective b) as (b'&<-).
  destruct (is_surjective b') as (a&<-).
  exists a;reflexivity.
Qed.

Global Instance bijective_id {A} : bijective (@id A).
Proof. split;split;unfold id;[|intro y;exists y];tauto. Qed.

Global Instance bijective_compose {A B C} (f : B -> C) (g : A -> B) :
  bijective f -> bijective g -> bijective (f ∘ g).
Proof.
  intros B1 B2;split;[apply injective_compose|apply surjective_compose];
    apply B1||apply B2.
Qed.

Instance injective_inl {A B} : injective (@inl A B).
Proof. split;intros e f E;inversion E;subst;reflexivity. Qed.
Instance injective_inr {A B} : injective (@inr A B).
Proof. split;intros e f E;inversion E;subst;reflexivity. Qed.


(** * Iterates of a function (Church numerals) *)  
Fixpoint iter_fun {A : Set} (f : A -> A) n :=
  match n with
  | 0 => id
  | S n => f ∘ (iter_fun f n)
  end.

Lemma iter_fun_plus {A:Set}(f : A -> A) n m x :
  iter_fun f (n+m) x = iter_fun f n (iter_fun f m x).
Proof. induction n;simpl;unfold Basics.compose;reflexivity||congruence. Qed.

Lemma iter_fun_times {A:Set}(f : A -> A) n m x :
  iter_fun f (n*m) x = iter_fun (iter_fun f m) n x.
Proof.
  induction n;simpl;unfold Basics.compose;[reflexivity|].
  rewrite <- IHn,iter_fun_plus;reflexivity.
Qed.

Lemma iter_fun_fix {A:Set}(f : A -> A) n x : f x = x -> iter_fun f n x = x.
Proof. intros E;induction n;[|simpl;unfold Basics.compose;rewrite IHn,E];reflexivity. Qed.
...
(** * Sets of sets (encoded as lists) *)
Section sets_of_sets.
  Context {A : Set} {decA : decidable_set A}.

  Global Instance smaller_sets : SemSmaller (list (list A)) :=
    fun l m => forall a, a ∈ l -> exists b, b ∈ m /\ a ≈ b.
  Global Instance equiv_sets : SemEquiv (list (list A)) :=
    fun l m => l ≲ m /\ m ≲ l.

  Global Instance smaller_sets_PreOrder : PreOrder ssmaller.
  Proof.
    split.
    - intros l a Ia;exists a;split;[assumption|reflexivity].
    - intros l m n E1 E2 a Ia;apply E1 in Ia as (b&Ib&Eb);apply E2 in Ib as (c&Ic&Ec);
        exists c;split;[assumption|rewrite Eb,Ec;reflexivity].
  Qed.

  Global Instance equiv_sets_Equivalence: Equivalence sequiv.
  Proof.
    split.
    - split;reflexivity.
    - intros l m (I1&I2);split;assumption.
    - intros l m n (I1&I2)(I3&I4);split;etransitivity;eassumption.
  Qed.

  Global Instance smaller_sets_PartialOrder: PartialOrder sequiv ssmaller.
  Proof. intros l m;split;unfold Basics.flip;intros (h1&h2);split;assumption. Qed.

  Global Instance incl_smaller_sets : subrelation (@incl (list A)) ssmaller.
  Proof. intros l m I a Ia;exists a;split;[apply I,Ia|reflexivity]. Qed.

  Global Instance equivalent_equiv_sets : subrelation (@equivalent (list A)) sequiv.
  Proof. intros l m I;split;apply incl_smaller_sets;rewrite I;reflexivity. Qed.

  Global Instance smaller_set_rev_Proper: Proper (ssmaller ==> ssmaller) (@rev (list A)).
  Proof.
    intros l m E a Ia;rewrite <- rev_equivalent in Ia;apply E in Ia as (b&Ib&Eb).
    exists b;split;[rewrite <- rev_equivalent|];assumption.
  Qed.
  Global Instance equiv_set_rev_Proper: Proper (sequiv ==> sequiv) (@rev (list A)).
  Proof. intros l m E;split;apply smaller_set_rev_Proper,E. Qed.
                    
  Global Instance smaller_set_cons_Proper :
    Proper (equivalent (A:=A) ==> ssmaller ==> ssmaller) cons.
  Proof.
    intros x y E l m E' b [<-|Ib].
    - exists y;split;[now left|assumption].
    - apply E' in Ib as (c&Ic&Ec);exists c;split;[now right|assumption].
  Qed.
  Global Instance equiv_set_cons_Proper :
    Proper (equivalent (A:=A) ==> sequiv ==> sequiv) cons.
  Proof.
    intros x y E l m E';split;apply smaller_set_cons_Proper,E';rewrite E;reflexivity.
  Qed.
  
  Global Instance filter_smaller_set_Proper f :
    Proper (@equivalent A ==> Logic.eq) f -> Proper (ssmaller ==> ssmaller) (filter f).
  Proof.
    intros P l m E a Ia;apply filter_In in Ia as (Ia&Fa).
    apply E in Ia as (b&Ib&Eb);rewrite Eb in Fa;exists b;split;[apply filter_In|];tauto.
  Qed.
  Global Instance filter_equiv_set_Proper f :
    Proper (@equivalent A ==> Logic.eq) f -> Proper (sequiv ==> sequiv) (filter f).
  Proof. intros P l m E;split;apply filter_smaller_set_Proper,E;assumption. Qed.
  
  Global Instance smaller_set_app_Proper:
    Proper (ssmaller ==> ssmaller ==> ssmaller) (@app _).
  Proof.
    intros l m E l' m' E' a Ia; apply in_app_iff in Ia as [Ia|Ia];
      [apply E in Ia|apply E' in Ia];destruct Ia as (b&Ib&Eb);
        exists b;(split;[apply in_app_iff|];tauto).
  Qed.
  Global Instance equiv_set_app_Proper:
    Proper (sequiv ==> sequiv ==> sequiv) (@app _).
  Proof. intros l m E l' m' E';split;apply smaller_set_app_Proper;apply E||apply E'. Qed.

  Definition smaller_sets_b (l m : list (list A)) :=
    forallb (fun a => existsb (fun b => equivalentb a b) m) l.
  Definition equiv_sets_b l m := smaller_sets_b l m && smaller_sets_b m l.
  
  Lemma smaller_sets_b_spec l m : smaller_sets_b l m = true <-> l ≲ m.
  Proof.
    unfold smaller_sets_b,ssmaller,smaller_sets.
  ...
  Corollary smaller_sets_dec l m : {l ≲ m} + {~( l ≲ m ) }.
  Proof.
    case_eq (smaller_sets_b l m);intro S;[left|right];
      revert S;[|rewrite <- not_true_iff_false];rewrite smaller_sets_b_spec;tauto.
  Qed.
  
  Lemma equiv_sets_b_spec l m : equiv_sets_b l m = true <-> l ≃ m.
  Proof.
    unfold equiv_sets_b,sequiv,equiv_sets.
    rewrite andb_true_iff.
    repeat rewrite smaller_sets_b_spec.
    reflexivity.
  Qed.

  Corollary equiv_sets_dec l m : {l ≃ m} + {~( l ≃ m ) }.
  Proof.
    case_eq (equiv_sets_b l m);intro S;[left|right];
      revert S;[|rewrite <- not_true_iff_false];rewrite equiv_sets_b_spec;tauto.
  Qed.
  
  Lemma smaller_sets_singl x l : x ∈ l -> [x]≲l.
  Proof.
    intros Ix ? [<-|F];[exists x;split;[assumption|reflexivity]|exfalso;apply F].
  Qed.
  
End sets_of_sets.

Lemma map_smaller_set_Proper (A B : Set) (f : list A -> list B):
  Proper (@equivalent A ==> @equivalent B) f ->  Proper (ssmaller ==> ssmaller) (map f).
Proof.
  intros P l m E a' Ia';apply in_map_iff in Ia' as (a&<-&Ia).
  apply E in Ia as (b&Ib&Eb);exists (f b);split;[apply in_map_iff;exists b;tauto|apply P,Eb].
Qed.
Lemma map_equiv_set_Proper (A B : Set) (f : list A -> list B):
  Proper (@equivalent A ==> @equivalent B) f ->  Proper (sequiv ==> sequiv) (map f).
Proof. intros P l m E;split;apply map_smaller_set_Proper,E;assumption. Qed.
 
Global Instance map_map_smaller_set_Proper (A B : Set) (f : A -> B) :
  Proper (ssmaller ==> ssmaller) (map (map f)).
Proof. apply map_smaller_set_Proper,map_equivalent_Proper. Qed.
Global Instance map_map_equiv_set_Proper (A B : Set) (f : A -> B) :
  Proper (sequiv ==> sequiv) (map (map f)).
Proof. apply map_equiv_set_Proper,map_equivalent_Proper. Qed.

Global Instance map_filter_smaller_set_Proper (A : Set) (f : A -> bool) :
  Proper (ssmaller ==> ssmaller) (map (filter f)).
Proof. apply map_smaller_set_Proper,filter_equivalent_Proper. Qed.
Global Instance map_filter_equiv_set_Proper (A : Set) (f : A -> bool) :
  Proper (sequiv ==> sequiv) (map (filter f)).
Proof. apply map_equiv_set_Proper,filter_equivalent_Proper. Qed.
         
Section noDup_upto_equiv.
  Context {A : Set} {decA : decidable_set A}.
  
  Fixpoint NoDupEq (l : list (list A)) : Prop :=
    match l with
    | [] => True
    | a::l => (forall b, b ∈ l -> ~ (a ≈ b)) /\ NoDupEq l
    end.

  Lemma NoDupEq_NoDup l : NoDupEq l -> NoDup l.
  Proof.
    induction l;[intros;apply NoDup_nil|].
    intros (nI&ih);apply NoDup_cons;[|auto].
    intros Ia;apply (nI a Ia);reflexivity.
  Qed.
  
  Lemma NoDupEq_equivalent_eq l a b : NoDupEq l -> a ∈ l -> b ∈ l -> a ≈ b -> a = b.
  Proof.
    intro nd; revert a b;induction l;[simpl;tauto|].
    simpl in nd;destruct nd as (h&nd).
    intros x y [<-|Ia] [<-|Ib] E.
    - reflexivity.
    - exfalso;apply (h y Ib E).
    - exfalso;apply (h x Ia);symmetry;assumption.
    - apply IHl;auto.
  Qed.
    
  Fixpoint undupEq (l : list (list A)) :=
    match l with
    | [] => []
    | a::l => if forallb (fun b => negb (equivalentb a b)) l then a :: undupEq l else undupEq l
    end.

  Lemma undupEq_spec (l : list (list A)) :
    l ≃ undupEq l
    /\ undupEq l ⊆ l
    /\ NoDupEq (undupEq l).
  Proof.
    cut (l ≲ undupEq l /\ undupEq l ⊆ l /\ NoDupEq (undupEq l));
      [intros (h1&h2&h3);repeat split;try assumption;apply incl_smaller_sets,h2|].
    induction l;simpl.
    - split;[reflexivity|split;[reflexivity|tauto]].
    - destruct IHl as (h1&h2&nd).
      case_eq (forallb (fun b => negb (equivalentb a b)) l);intros h.
      + rewrite forallb_forall in h.
        ...
        split;[|split].
        * intros c [<-|Ic].
          -- exists a;split;[now left|reflexivity].
          -- apply h1 in Ic as (b&Ib&Eb).
             exists b;split;[right|];assumption.
        * rewrite h2;reflexivity.
        * split;[|assumption].
          intros b Ib;apply h,h2,Ib.
      + apply forallb_false_exists in h as (b&Ib&Eb).
        apply negb_false_iff,equivalentb_spec in Eb.
        split;[|split].
        * intros c [<-|Ic].
          -- apply h1 in Ib as (c&Ic&Ec).
             exists c;split;[|etransitivity];eauto.
          -- apply h1,Ic.
        * intros x Ix;right;apply h2,Ix.
        * assumption.
  Qed.

  Lemma undupEq_eq l : l ≃ undupEq l.
  Proof. apply undupEq_spec. Qed.
  
  Lemma undupEq_NoDupEq l : NoDupEq (undupEq l).
  Proof. apply undupEq_spec. Qed.

  Lemma undupEq_NoDupEq_id l : NoDupEq l -> undupEq l = l.
  Proof.
    ...
    apply forallb_forall in E as ->.
    reflexivity.
  Qed.
  
  Lemma undupEq_smaller L : ⎢undupEq L⎥ <= ⎢L⎥.
  Proof.
    induction L as [|a L];[reflexivity|].
    simpl.
    destruct (forallb _ _);rsimpl;lia.
  Qed.
  
End noDup_upto_equiv.

Lemma undupEq_map {A B : Set} {decA : decidable_set A} {decB : decidable_set B}
      (f : A -> B) L :
  injective f -> undupEq (map (map f) L) = map (map f) (undupEq L).
Proof.
  intros inj;induction L as [|a L];simpl;[reflexivity|].
  case_eq (forallb (fun b : list A => negb (equivalentb a b)) L).
  - intros E.
    replace (forallb _ _) with true;[rewrite IHL;reflexivity|].
    symmetry;apply forallb_forall.
    intros x Ix;apply in_map_iff in Ix as (b&<-&Ib).
    rewrite forallb_forall in E;apply E in Ib.
    rewrite negb_true_iff,<-not_true_iff_false,equivalentb_spec in *.
    rewrite equivalentb_spec in Ib.
    intros F;apply Ib.
    intro x;split;intros Ix;[cut (f x ∈ map f a)|cut (f x ∈ map f b)];
      try (apply in_map_iff;exists x;tauto);clear Ix;intros Ix;
        apply F,in_map_iff in Ix as (y&Ey&Iy);apply is_injective in Ey as ->;assumption.
  - intros E.
    replace (forallb _ _) with false;[rewrite IHL;reflexivity|].
    apply forallb_false_exists in E as (b&Ib&Fb).
    symmetry;apply forallb_false_exists.
    exists (map f b);split.
    + apply in_map_iff;exists b;tauto.
    + rewrite negb_false_iff,equivalentb_spec in *.
      rewrite equivalentb_spec in Fb.
